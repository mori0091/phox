...
fn test_let_simple() {
    // let x = 10 ; x + 1
    let (val, sch) = eval("let x = 10 ; x + 1").unwrap();
...
fn test_let_tuple() {
    // let t = (1, Some 2) ; t
    let (val, sch) = eval("let t = (1, Some 2) ; t").unwrap();
...
fn test_let_polymorphism_id() {
    ...
    let (val, sch) = eval("let id = \\x. x ; (id 1, id true)").unwrap();
...
#[test]
fn test_pat_var() {
    let (val, sch) = eval("let x = 42 ; x").unwrap();
    assert_eq!(format!("{}", val), "42");
    assert_eq!(format!("{}", sch.pretty()), "Int");
}
...
fn test_let_and_use() {
    ...
    let (val, sch) = eval("let x = 5 ; let y = x * 2 ; y + 3").unwrap();
...
fn test_nested_block_top_level() {
    ...
    let (val, sch) = eval("let x = { let y = 2 ; y + 1 } ; x * 10").unwrap();
